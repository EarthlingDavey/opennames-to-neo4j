...
const getRedisUri = () => {
  /*
   * Redis uri specified as environment variables
   */
  if (process.env.REDIS_URI) {
    debug('redis uri, using env variables');
  ...
  if (result?.parsed?.REDIS_URI) {
    debug('redis uri, using dotenv env variables');
    return result.parsed.REDIS_URI;
  }

  debug('redis uri, using defaults values');
  return 'redis://redis:6379';
};

export { getCredentials, getRedisUri };
